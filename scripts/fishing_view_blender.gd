...
# Composition knobs
@export var back_distance: float = 4.5     # how far behind the player along the water direction
@export var height: float = 2.2            # lift camera up
@export var side_offset: float = 1.6       # + moves cam to player's right (player appears left)
@export var pitch_deg: float = -12.0       # look-down
...
func enter_fishing_view(player: Node3D, water_forward: Vector3, anchor: Node3D = null) -> void:
	print("[FishCam] enter_fishing_view called")

	_in_fishing = true

	# IMPORTANT: snapshot AFTER follow is disabled by the controller
	_pre_fish_xform = global_transform
	...
	# Target look point (rename to avoid shadowing Node3D.look_at)
	var focus := base_pos + fwd * 2.5

	# Build target transform (use Basis.looking_at to avoid the warning)
